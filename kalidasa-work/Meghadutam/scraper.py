...
def extract_verses(soup: BeautifulSoup) -> List[Dict[str, str]]:
    """
    Extract verses from the Meghadutam webpage.
    ...
    # Find all text that matches the verse pattern (e.g., "॥ १.१॥" or "॥ ? ॥")
    ...
    for i in range(0, len(verse_parts)-1, 2):
        ...
        # Handle verses marked with "?"
        if verse_number == "?":
            question_mark_count += 1
            verse_number = str(question_mark_count)
...
def main():
    # URL for Meghadutam
    url = "https://sanskritdocuments.org/doc_z_misc_major_works/meghanew.html"
