...
def extract_verses(soup: BeautifulSoup) -> List[Dict[str, str]]:
    """
    Extract verses from the Kumarasambhavam webpage.
    ...
    # Find all text that matches the verse pattern (e.g., "॥ १।१॥")
...
def convert_devanagari_to_english(number: str) -> str:
    ...
    Args:
        number (str): Devanagari number string (e.g., "१।१")
        
    Returns:
        str: English number string (e.g., "1.1")
    """
    devanagari_to_english = {
        '०': '0', '१': '1', '२': '2', '३': '3', '४': '4',
        '५': '5', '६': '6', '७': '7', '८': '8', '९': '9'
    }
    # Convert Devanagari numbers and replace danda (।) with period
    return ''.join(devanagari_to_english.get(c, c) for c in number).replace('।', '.')
...
def process_url(url: str) -> List[Dict[str, str]]:
    ...
    for verse in verses:
        ...
        ref = f'कुमारसम्भवम्->{english_number}'
...
def main():
    # URL for Kumarasambhavam
    url = "https://sanskritdocuments.org/doc_z_misc_major_works/kumArasambhavam8.html"
