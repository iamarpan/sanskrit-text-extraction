...
def extract_verses(soup: BeautifulSoup) -> List[Dict[str, str]]:
    """
    Extract verses from the Raghuvansha webpage.
    ...
    # Find all text that matches the verse pattern (e.g., "॥ ४-१॥")
...
def process_url(url: str) -> List[Dict[str, str]]:
    ...
    for verse in verses:
        ...
        ref = f'रघुवंशं->{english_number.replace("-", ".")}'  # Replace hyphen with period in the reference
...
def main():
    # Generate URLs for all 19 chapters
    base_url = "https://sanskritdocuments.org/doc_z_misc_major_works/raghuvansha{}.html"
    urls = [base_url.format(i) for i in range(1, 20)]
